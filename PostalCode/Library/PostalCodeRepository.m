...
#define DATABASE_NAME    @"data_201809.sqlite"
...
+ (NSMutableArray *)getFavorites
{
    ...
    return [[NSMutableArray alloc] initWithArray:[defaults arrayForKey:FAVORITE]];
}
...
+ (void)deleteFavoritePostalCodeModel:(NSInteger)index
{
    ...
    if (favorite.count > index) {
        [favorite removeObjectAtIndex:index];
        [defaults setObject:[favorite copy] forKey:FAVORITE];
        [defaults synchronize];
    }
}
...
{
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [defaults setObject:[NSArray new] forKey:FAVORITE];
    [defaults synchronize];
}
